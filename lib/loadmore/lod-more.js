document.addEventListener('DOMContentLoaded', function () {
  const loadMoreButtons = document.querySelectorAll('[id^="lod-more"]'); // Select all "Load More" buttons
  const sections = ['#excel', '#powerBI']; // Define sections to handle
  ...
  sections.forEach((section, index) => {
      ...
      const loadMoreButton = document.getElementById(`lod-more-${index}`); // Assume unique button IDs like 'lod-more-0', 'lod-more-1'
      ...
      if (loadMoreButton) {
          loadMoreButton.addEventListener('click', function () {
              ...
              const remainingHiddenItems = document.querySelectorAll(`${section} .project-item.hidden`).length;
